...
function logout() {
  ...
  if (dataEntryButton) dataEntryButton.classList.add("hidden");
}
...
function startForm(rowId) {
  ...
  const formElement = document.getElementById(`form${rowId}`);
  const addRowButton = document.getElementById('addRowButton');
  if (formElement && addRowButton) {
    isFormActive = true;
    formElement.classList.remove('hidden');
    addRowButton.classList.add('disabled');
  } else {
    console.error(`Element with ID form${rowId} or addRowButton not found`);
  }
}
...
function updateAccountOptions(rowId) {
  ...
  if (sourceAccountSelect && destAccountSelect) {
...
function toggleInvoice(rowId) {
  ...
  const invoiceInput = document.getElementById(`invoice${rowId}`);
  if (invoiceInput) {
    invoiceInput.disabled = noInvoice;
    if (noInvoice) invoiceInput.value = '';
  }
}

function toggleDocument(rowId) {
  const noDocument = document.getElementById(`noDocument${rowId}`).checked;
  const documentInput = document.getElementById(`document${rowId}`);
  if (documentInput) {
    documentInput.disabled = noDocument;
    if (noDocument) documentInput.value = '';
  }
}
...
function nextStep(rowId, currentStep, nextStep) {
  ...
  const currentStepDiv = document.getElementById(`step${currentStep}_${rowId}`);
  const nextStepDiv = document.getElementById(`step${nextStep}_${rowId}`);
...
function goBack(rowId, currentStep, prevStep) {
  const currentStepDiv = document.getElementById(`step${currentStep}_${rowId}`);
  const prevStepDiv = document.getElementById(`step${prevStep}_${rowId}`);
...
function deleteRow(rowId) {
  if (confirm('آیا از حذف این ردیف مطمئن هستید؟')) {
    ...
    isFormActive = false;
    const addRowButton = document.getElementById('addRowButton');
    if (addRowButton) addRowButton.classList.remove('disabled');
...
function editTransaction(rowId) {
  ...
  isFormActive = true;
  const addRowButton = document.getElementById('addRowButton');
  if (addRowButton) addRowButton.classList.add('disabled');
